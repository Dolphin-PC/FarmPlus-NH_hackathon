...
const ContractDialog = (props) => {
  ...
  const { onClose, open, requester, product, tradeId, deposit, isContract } = props;
  ...
  const handleOnAccept = () => {
    if (!isContractAgree) return alert("계약 정보 확인 및 동의가 필요합니다.");
    ...
    return (
      <Accordion>
        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
          <FormControlLabel control={<Checkbox checked={check} name={agreeName} onChange={handleOnAgree} />} label={summary} />
        </AccordionSummary>
        <AccordionDetails>
  ...
  switch (page) {
    ...
    case "contract":
      return (
        <Dialog open={open} onClose={handleClose}>
          ...
          <DialogContent>
            <p>아래 목적물을 포전매매 함에 있어 매도인(이하 “갑”이라고 한다)과 매수인(이하 “을”이라고 한다)은 다음과 같이 계약을 체결하고 신의성실의 원칙에 따라 이를 이행하여야 한다.</p>
            <ContractAccordionComp product={product} requester={requester} />
            {contract.map((agree, index) => {
              return <AccordionRender key={index} {...agree} />;
            })}
            <small>
              [주의사항] 농수산물유통 및 가격안정에 관한 법률 제90조제1항제2호에 따라 이 표준계약서와 다른 계약서를 사용하면서 ‘표준계약서’로 거짓표시 하거나, ‘농림축산식품부’ 및 ‘농림축산식품부
              표식’을 사용하는 매수인에게는 1천만원 이하의 과태료가 부과됩니다.
            </small>
          </DialogContent>
      ...
      if (user.id === product.seller.id) {
        return (
          <Dialog open={open} onClose={handleClose}>
            ...
            <DialogContent>상대방의 계약금이 아직 입금되지 않았습니다.</DialogContent>
            <DialogActions>
        ...
        <Drawer anchor="bottom" open={open} onClose={handleClose}>
          <div style={{ padding: "20px 20px 0px 20px " }}>
            ...
            <div className="Account-Bank-Box">
              ...
              <DrawerText left="전화번호" right={product.seller.phoneNumber} />
              <DrawerText left="가격" right={`${product.cost.toLocaleString()} 원`} />
            </div>
          </div>

          <Button style={{ backgroundColor: Color.mainColor, color: "white" }} fullWidth onClick={handleOnSendDeposit} className="Deposit-Button">
            출금 이체
          </Button>
        </Drawer>
      );
    ...
    default:
      return (
        <DialogRender>
          <Button className="ContractButton" onClick={() => setPage("contract")} color="primary" variant="outlined">
            <h5>
              계약 정보 확인 및 동의 <Checkbox disabled checked={isContractAgree} />
            </h5>
          </Button>
        </DialogRender>
      );
