...
const DetailPageView = () => {
  ...
  const current = useSelector((state: RootStateType) => state.post.current);
      ...
      <div>
        ...
        >
          <div style={{ display: "flex", alignItems: "center" }}>
            ...
            <p style={{ margin: 0 }}>
              ...
              {current.seller.isVIP ? (
                <Tooltip title={longText} classes={{ tooltip: classes.customWidth }} arrow>
          ...
          <div style={{ marginTop: "auto" }}>
            {isHeart ? <FavoriteIcon style={{ fontSize: 30 }} onClick={handleOnFavorite} /> : <FavoriteBorder style={{ fontSize: 30 }} onClick={handleOnFavorite} />}
            &emsp;
    ...
    <div>
      <div style={{ padding: 20, marginBottom: 80 }}>
        ...
        <div className="Account-Bank-Box">
          <DrawerText left="크기" right={`${current.size.toLocaleString()} 평`} />
          <DrawerText left="가격" right={`${current.cost.toLocaleString()} 원`} />
  ...
  return (
    <div>
      <TopIconRender />
      <img src={current.imageUrls[0]} alt="#" style={{ height: height / 2, width: "100%", objectFit: "cover" }} />
      ...
      <Drawer anchor="bottom" open={openBottomDrawer} onClose={toggleDrawer(false)}>
        <DrawerContent />
      </Drawer>
    </div>
  );
